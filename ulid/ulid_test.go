...
import (
	...
	"errors"
	"fmt"
	"github.com/dyammarcano/utils/ulid"
	"io"
	"math"
	"math/rand"
	"strings"
...
func TestNew(t *testing.T) {
	...
	fn := func(ms uint64, e io.Reader) *ulid.ULID {
	...
	t.Run("ULID", testULID(fn))

	t.Run("Error", func(t *testing.T) {
		_, err := ulid.New(ulid.MaxTime()+1, nil)
		if got, want := err, ulid.ErrBigTime; !errors.Is(want, got) {
			t.Errorf("got err %v, want %v", got, want)
		}

		_, err = ulid.New(0, strings.NewReader(""))
		if got, want := err, io.EOF; !errors.Is(want, got) {
			t.Errorf("got err %v, want %v", got, want)
		}
	})
}
...
func testULID(mk func(uint64, io.Reader) *ulid.ULID) func(*testing.T) {
	return func(t *testing.T) {
		want := &ulid.ULID{0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x86, 0xa0, 0, 0, 0, 0, 0, 0, 0}
		...
		copy(want[6:], entropy)
		wantWithEntropy := want
		if got := mk(1e5, bytes.NewReader(entropy)); got != wantWithEntropy {
			t.Errorf("\ngot  %#v\nwant %#v", got, wantWithEntropy)
		}
	}
}
...
func TestMarshalingErrors(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			if got, want := tc.fn([]byte{}), tc.err; !errors.Is(want, got) {
				t.Errorf("got err %v, want %v", got, want)
			}
		})
...
func TestULIDTime(t *testing.T) {
	...
	var id ulid.ULID
	if got, want := id.SetTime(maxTime+1), ulid.ErrBigTime; !errors.Is(want, got) {
		t.Errorf("got err %v, want %v", got, want)
	}
...
func TestEntropy(t *testing.T) {
	...
	var id ulid.ULID
	if got, want := id.SetEntropy([]byte{}), ulid.ErrDataSize; !errors.Is(want, got) {
		t.Errorf("got err %v, want %v", got, want)
	}
...
func TestOverflowHandling(t *testing.T) {
	...
	for s, want := range map[string]error{
		...
		if _, have := ulid.Parse(s); !errors.Is(have, want) {
...
func TestMonotonicOverflow(t *testing.T) {
	...
	if have, want := err, ulid.ErrMonotonicOverflow; !errors.Is(want, have) {
